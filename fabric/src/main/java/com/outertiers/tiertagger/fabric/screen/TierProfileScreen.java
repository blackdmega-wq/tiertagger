...
public class TierProfileScreen extends Screen {
    ...
    // Smooth, frame-rate-independent pulse driven by wall-clock time.
    // pulse01() returns a value in [0,1] that oscillates with a 2.4 s period
    // — used for breathing borders, glow halos and tier-pill highlights.
    ...
    public void render(DrawContext ctx, int mouseX, int mouseY, float delta) {
        ...
        try {
            ...
            // (v1.21.11.55) Soft animated drop-shadow + breathing border so
            // the panel feels alive instead of static. The shadow ring is
            // painted underneath the fill, the border is painted twice — a
            // static mid-tone underneath and a brighter blue accent that
            // pulses in opacity on top.
            float pulse = pulse01();
    ...
    private void renderHeader(DrawContext ctx, int x, int y, int w, int h) {
        ...
        // (v1.21.11.55) Animated diagonal sheen sweep across the header so
    ...
    private void renderServiceCard(DrawContext ctx, TierService svc, ServiceData sd,
        ...
        // Card background with a pulsing accent glow on the left edge
        // (v1.21.11.55 — animated bloom that breathes with the rest of
        //  the panel).
        ...
        // Pulsing bloom that fades out across 12 px to the right
        for (int i = 0; i < 12; i++) {
        ...
        if (sd.fetchedAt == 0L) {
            ...
            if (best != null) {
                ...
                // (v1.21.11.55) Animated halo behind the best-tier pill so
                // it visually catches the eye — alpha breathes with the
                // global pulse and the colour echoes the tier accent.
