...
## What's new in 1.21.11.56

- **Live preview shows the full body.** The Tiers Config tab's *LIVE PREVIEW* card was clipping the rendered Minecraft skin — only the head (and sometimes a sliver of torso) made it onto the screen because the preview rect extended below the scroll area's clip line. The preview height was bumped back up to 240 px and the reserved scroll headroom was expanded so the entire figure (head → chest → arms → legs) is visible inside the card and fully scrollable above the bottom action row.
- **`/tiertagger compare` finally shows the OuterTiers logo in the centre square.** The 512×512 logo PNG was saved as 16-bit RGBA — the exact silent-rejection failure mode documented in the v1.7.9 release notes for the gamemode icons. Minecraft's `NativeImage` returned a null texture, the `try/catch` swallowed it, and the user saw an empty golden square between the two skin panels. The logo (and the mod icon) have been re-encoded to 8-bit RGBA so they actually load. The centre slot was simultaneously enlarged to 64×64, the logo is drawn with a 2-px inset and a soft dark backdrop for contrast, and the pulsing yellow halo is now rendered as concentric **border rings** instead of filled rectangles — so the glow stays *outside* the icon instead of painting on top of it.
- **OuterTiers gamemode icons now use the official artwork from outertiers.com.** All ten OuterTiers modes (`ogvanilla`, `vanilla`, `uhc`, `pot`, `nethop`, `smp`, `sword`, `axe`, `mace`, `speed`) ship a per-service icon override under `assets/tiertagger/textures/icons/outertiers/` so OuterTiers rows match the website exactly. Note: the website renamed *Vanilla* to *Crystal*, so the bundled `outertiers/vanilla.png` is the crystal artwork — same artwork the website displays for that mode.
- **Compare-screen polish.** The centre logo's halo amplitude was raised slightly (`60..140` instead of `40..100`) so the glow reads against bright header gradients without washing out the brand mark.
