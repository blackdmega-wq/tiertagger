...
public class TierTaggerFabricCommand {
        ...
        ClientCommandRegistrationCallback.EVENT.register((dispatcher, registryAccess) -> {
            dispatcher.register(ClientCommandManager.literal("tiertagger")
                ...
                .then(ClientCommandManager.literal("version").executes(c -> {
                    c.getSource().sendFeedback(Component.literal("§a[TierTagger] §rversion §e" + TierTaggerCore.MOD_VERSION));
                    return 1;
                }))
                ...
                .then(ClientCommandManager.literal("reload").executes(c -> {
                    ...
                    c.getSource().sendFeedback(Component.literal("§a[TierTagger] §rConfig reloaded from disk."));
                    return 1;
                }))
                .then(ClientCommandManager.literal("reset").executes(c -> {
                    ...
                    TierTaggerCore.cache().invalidate();
                    c.getSource().sendFeedback(Component.literal("§a[TierTagger] §rConfig reset to defaults."));
                    return 1;
                }))
                .then(ClientCommandManager.literal("ttl")
                    .then(ClientCommandManager.argument("seconds", IntegerArgumentType.integer(10, 86400))
                        .executes(c -> {
                            ...
                            TierTaggerCore.config().save();
                            c.getSource().sendFeedback(Component.literal("§a[TierTagger] §rCache TTL set to §e" + s + "s§r."));
                            return 1;
                        })))
                .then(ClientCommandManager.literal("color").executes(c -> {
                    ...
                    TierTaggerCore.config().save();
                    c.getSource().sendFeedback(Component.literal("§a[TierTagger] §rColoured badges: " +
                    ...
                    .then(ClientCommandManager.argument("style", StringArgumentType.word())
                        ...
                        .executes(c -> {
                            ...
                            if (!TierConfig.isValidBadgeFormat(f)) {
                                c.getSource().sendError(Component.literal("Unknown format: " + f + " (use bracket|plain|short)"));
                                return 0;
                            }
                            TierTaggerCore.config().badgeFormat = f;
                            TierTaggerCore.config().save();
                            c.getSource().sendFeedback(Component.literal("§a[TierTagger] §rBadge format: §e" + f));
                            return 1;
                        })))
                    ...
                    .then(ClientCommandManager.argument("player1", StringArgumentType.word())
                        .then(ClientCommandManager.argument("player2", StringArgumentType.word())
                            .executes(c -> {
                                ...
                                c.getSource().sendFeedback(Component.literal("§7[TierTagger] §rComparing §e" + n1 + " §rvs §e" + n2 + "§r…"));
                                return 1;
                            })
                            .then(ClientCommandManager.argument("tierlist", StringArgumentType.word())
                                ...
                                .executes(c -> {
                                    ...
                                    c.getSource().sendFeedback(Component.literal("§7[TierTagger] §rComparing §e" + cn1 + " §rvs §e" + cn2 + " §7(" + tlMsg + ")§r…"));
                                    return 1;
                                })))))
                .then(ClientCommandManager.literal("clear")
                    .then(ClientCommandManager.argument("player", StringArgumentType.word())
                        .executes(c -> {
                            ...
                            c.getSource().sendFeedback(Component.literal("§a[TierTagger] §rCleared cache for §e" + name));
                            return 1;
                        })))
                    ...
                    .then(ClientCommandManager.literal("left")
                        .then(ClientCommandManager.argument("name", StringArgumentType.word())
                            ...
                            .executes(c -> {
                                ...
                                if (svc == null) { c.getSource().sendError(Component.literal("Unknown service: " + s)); return 0; }
                                TierTaggerCore.config().leftService = svc.id;
                                TierTaggerCore.config().save();
                                c.getSource().sendFeedback(Component.literal("§a[TierTagger] §rLeft badge: §e" + svc.displayName));
                                return 1;
                            })))
                    .then(ClientCommandManager.literal("right")
                        .then(ClientCommandManager.argument("name", StringArgumentType.word())
                            ...
                            .executes(c -> {
                                ...
                                TierTaggerCore.config().save();
                                c.getSource().sendFeedback(Component.literal("§a[TierTagger] §rRight badge: §e" + svc.displayName));
                                return 1;
                            })))
                    .then(ClientCommandManager.literal("toggle")
                        .then(ClientCommandManager.argument("name", StringArgumentType.word())
                            ...
                            .executes(c -> {
                                ...
                                TierTaggerCore.config().save();
                                c.getSource().sendFeedback(Component.literal("§a[TierTagger] §r" + svc.displayName + ": " +
                                    (!cur ? "§aon" : "§coff")));
                                return 1;
                            }))))
                .then(ClientCommandManager.literal("toggle").executes(ctx -> {
                    ...
                    TierTaggerCore.config().save();
                    ctx.getSource().sendFeedback(Component.literal("§a[TierTagger] §rTab badges: " +
                ...
                .then(ClientCommandManager.literal("nametag").executes(ctx -> {
                    ...
                    TierTaggerCore.config().save();
                    ctx.getSource().sendFeedback(Component.literal("§a[TierTagger] §rNametag badges: " +
                ...
                .then(ClientCommandManager.literal("peak").executes(ctx -> {
                    ...
                    TierTaggerCore.config().save();
                    TierTaggerCore.cache().invalidate();
                    ctx.getSource().sendFeedback(Component.literal("§a[TierTagger] §rShow peak tier: " +
                        (TierTaggerCore.config().showPeak ? "§aon" : "§coff")));
                    return 1;
                }))
                .then(ClientCommandManager.literal("refresh").executes(ctx -> {
                    TierTaggerCore.cache().invalidate();
                    ctx.getSource().sendFeedback(Component.literal("§a[TierTagger] §rCache cleared."));
                    return 1;
                }))
                .then(ClientCommandManager.literal("config").executes(ctx -> {
                    PendingScreen.open(new TierConfigScreen(null));
                    ctx.getSource().sendFeedback(Component.literal("§7[TierTagger] §rOpening config…"));
                    return 1;
                }))
                // /tiertagger search <player> — opens the player lookup UI.
                // Renamed from the old "profile" subcommand per user request;
                // the old "profile" literal has been fully removed (use
                // "search" instead).
                .then(ClientCommandManager.literal("search")
                    .then(ClientCommandManager.argument("player", StringArgumentType.word())
                        .executes(ctx -> {
                            ...
                            ctx.getSource().sendFeedback(Component.literal("§7[TierTagger] §rSearching for §e" + name + "§r…"));
                            return 1;
                        })
                        .then(ClientCommandManager.argument("tierlist", StringArgumentType.word())
                            .suggests((c, b) -> {
                            ...
                            .executes(ctx -> {
                                ...
                                String tlMsg = (only == null) ? "all tier lists" : only.displayName + " only";
                                ctx.getSource().sendFeedback(Component.literal("§7[TierTagger] §rSearching for §e" + name + " §7(" + tlMsg + ")§r…"));
                                return 1;
                            }))))
    ...
    private static void sendLookup(FabricClientCommandSource src, String name) {
        ...
        if (opt.isEmpty()) {
            src.sendFeedback(Component.literal("§7[TierTagger] §rFetching §e" + name + "§r…"));
            return;
        }
        PlayerData data = opt.get();
        src.sendFeedback(Component.literal("§6§l━━━━━━━━━ §f§lTier Lookup §6§l━━━━━━━━━"));
        src.sendFeedback(Component.literal(" §b§l" + name));
        src.sendFeedback(Component.literal("§8─────────────────────────────────────"));
        for (TierService svc : TierService.values()) {
            ...
            src.sendFeedback(Component.literal(String.format(" §f%-12s§r %s", svc.displayName, state)));
        }
    }
    ...
    private static void sendCompare(FabricClientCommandSource src, String n1, String n2, String tierlistArg) {
        ...
        if (e1.isEmpty() || e2.isEmpty()) {
            src.sendFeedback(Component.literal("§7[TierTagger] §rFetching tiers — try again in a moment…"));
            return;
        }
        ...
        if (!want.equals("all") && !want.isBlank()) {
            ...
            if (only == null) {
                src.sendError(Component.literal("Unknown tier list: " + tierlistArg +
        ...
        src.sendFeedback(Component.literal("§6§l━━━━━━━━━ §f§l" + header + " §6§l━━━━━━━━━"));
        src.sendFeedback(Component.literal(String.format(" §b§l%-12s §7vs §b§l%s", n1, n2)));
        src.sendFeedback(Component.literal("§8─────────────────────────────────────"));
        ...
        for (TierService svc : TierService.values()) {
            ...
            for (String mode : svc.modes) {
                ...
                src.sendFeedback(Component.literal(String.format(" §7%-3s§r §f%-9s §r%s §r %s§r %s§r",
                    svc.shortLabel, mode, c1, marker, c2)));
            }
        }
        src.sendFeedback(Component.literal("§8─────────────────────────────────────"));
        src.sendFeedback(Component.literal(String.format(" §7Wins: §a%s§7: §f%d  §7- §a%s§7: §f%d  §7(§e%d tie%s§7)",
    ...
    private static void sendHelp(FabricClientCommandSource src) {
        src.sendFeedback(Component.literal("§a===== §fTierTagger Commands §a=====").formatted(ChatFormatting.RESET));
        String[][] rows = {
        ...
        for (String[] r : rows) src.sendFeedback(Component.literal("§e" + r[0] + " §8— §7" + r[1]));
        src.sendFeedback(Component.literal("§7Services: §emctiers §8/ §eoutertiers §8/ §epvptiers §8/ §esubtiers"));
    }
}
